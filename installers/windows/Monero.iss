; Mangonote Fluorine Fermi GUI Wallet Installer for Windows
; Copyright (c) 2017-2020, The Monero Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\mangonote-wallet-gui.exe")

[Setup]
AppName=Mangonote GUI Wallet
...
DefaultDirName={commonpf}\Mangonote GUI Wallet
DefaultGroupName=Mangonote GUI Wallet
UninstallDisplayIcon={app}\mangonote-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Mangonote Developer Community
AppPublisherURL=https://getmangonote.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Mangonote files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Mangonote.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Mangonote GUI wallet exe and guide
Source: "bin\mangonote-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\mangonote-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Mangonote CLI wallet
Source: "bin\extras\mangonote-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Mangonote wallet RPC interface implementation
Source: "bin\extras\mangonote-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Mangonote daemon
Source: "bin\mangonoted.exe"; DestDir: "{app}"; Flags: ignoreversion

; Mangonote daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "mangonote-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Mangonote blockchain utilities
Source: "bin\extras\mangonote-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\mangonote-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\mangonote-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitmangonote');
  s := 'The default folder to store the Mangonote blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\mangonote-project\mangonote-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Mangonote default location
...
begin
  Result := BlockChainDir('') + '\bitmangonote.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "mangonote-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Mangonote daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\mangonoted.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\mangonote-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Mangonote GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\mangonote-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\mangonote-gui-wallet-guide.pdf"; IconFilename: "{app}\mangonote-wallet-gui.exe"
...
Name: "{group}\Utilities\Mangonote Daemon"; Filename: "{app}\mangonoted.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\mangonote-wallet-cli.exe"; WorkingDir: "{userdocs}\Mangonote\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Mangonote\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\mangonote-wallet-gui\mangonote-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\mangonote-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM mangonoted.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\mangonote-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\mangonote-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\mangonote-project\mangonote-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\mangonote-project\mangonote-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "mangonote:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "mangonote://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "mangonote"; ValueType: "string"; ValueData: "URL:Mangonote Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "mangonote"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "mangonote\DefaultIcon"; ValueType: "string"; ValueData: "{app}\mangonote-wallet-gui.exe,0"
Root: HKCR; Subkey: "mangonote\shell\open\command"; ValueType: "string"; ValueData: """{app}\mangonote-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "mangonoteseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "mangonoteseed://a.b.c.d:port"
Root: HKCR; Subkey: "mangonoteseed"; ValueType: "string"; ValueData: "URL:Mangonote Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "mangonoteseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "mangonoteseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\mangonote-wallet-gui.exe,0"
Root: HKCR; Subkey: "mangonoteseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\mangonote-wallet-gui.exe"" ""%1"""
